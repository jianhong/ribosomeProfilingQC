...
can help user to determine the real best Psite. 
It is shown here that the start codon is enriched in position -9 from 5' end of reads and
in position 19 from 3' end of reads. That means there are lots of ribosome are docking at translation start position.
...
Metagene plot can indicates the reads distribution in 5'UTR, CDS and 3'UTR region.
...
These plots can help you to double check the p-site position is correct or not. If it is correct, most of the reads should be assigned to frame0.
...
To determine how many of my raw reads are mapping with a P site in frame 0.
...
or get the online file infor via `AnnotationHub`.
