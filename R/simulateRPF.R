...
simulateRPF <- function(txdb, outPath, genome, samples = 6,
  ...
  p <- lapply(p, function(.sample){
    ...
    .sample <- shift(.sample, shift = 1) ## shift 1
  ...
  ## split the de 5'UTR and 3'UTRs into up and down group
  ...
  if(length(cds_de$CDS)>0){
    ...
    de_cnt <- countOverlaps(de_evt_rg, pp)/2
    ...
    de_evt_ol <- findOverlaps(de_evt_rg, pp)
    ...
    de_evt_reads <- mapply(de_to_move[as.numeric(names(de_evt_reads))], de_evt_reads,
    ...
    gp <- de_evt_reads$group
    de_evt_reads$group <- NULL
    p <- split(de_evt_reads, gp)
  }
  ...
  gl <- lapply(p, function(.sample){
    ...
    if(!mis_genome){
      ...
      if(includeReadsSeq){
        seqs <- DNAStringSet(x=character(length = length(reads)))
        for(i in unique(mcols(reads)$tx)){
          k <- mcols(reads)$tx==i
          seq <- getSeq(genome, as(reads[k], "GRanges"))
          seq <- sequenceLayer(seq, cigar(reads)[k],
                               from = "reference",
  ...
  # grn <- gl[[1]]
  # seq2 <- as.character(grn[mcols(grn)$tx %in% "ENSDART00000166393"])
  # all(seq01==seq2)
  ...
  # seq0 <- paste(as.character(seq), collapse="")
  # seq01 <- sapply(re, function(.re) substr(seq0, start = .re, stop = .re+readsLen-1))
  # seq2 <- as.character(grn[mcols(grn)$tx %in% "ENSDART00000159144"])
  # all(seq01==seq2)
  if(!missing(outPath)){
    dir.create(outPath)
