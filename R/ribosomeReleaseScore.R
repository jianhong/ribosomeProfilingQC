...
#' @description RRS is calculated as the ratio of translational efficiency in the CDS with RPFs in the 3'UTR.
#' @param cdsTE,utr3TE translational efficiency of CDS and UTR3 region. Output of \link{translationalEfficiency}
#' @param CDSsampleOrder,UTR3sampleOrder sample order of cdsTE and utr3TE The parameters
#' are used to make sure that the order of CDS and UTR3 in TE is corresponding samples.
...
ribosomeReleaseScore <- function(cdsTE, utr3TE, CDSsampleOrder, UTR3sampleOrder,
  ...
  if(!all(c("RPFs", "mRNA", "TE") %in% names(cdsTE)) ||
    ...
    stop("cdsTE and utr3TE must be output of translationalEfficiency And RPFs, mRNA and TE must be available.")
  }
  cdsTE <- cdsTE$TE
  utr3TE <- utr3TE$TE
